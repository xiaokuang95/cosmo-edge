...
bool PassFlowOsdDraw(VideoFramePtr frame, const std::vector<MsgTaskArea>& areas, int enter, int leave) {
    ...
    osd.OSDDrawTextEx(x, y, cfg.enterLabel + " " + std::to_string(enter), textColor, cfg.fontSize, {0, 0, 0}, 0,
                      true, 0);
    osd.OSDDrawTextEx(x, y + cfg.fontSize + 18, cfg.leaveLabel + " " + std::to_string(leave), textColor,
